...
// https://github.com/Akebi-Group/Akebi-GC/blob/master/cheat-base/src/cheat-base/HookManager.h
...
public:
	...
	static Fn getOrigin(Fn handler, const char* callerName = nullptr) noexcept
	{
		if (holderMap.count(reinterpret_cast<void*>(handler)) == 0) {
			printf("Origin not found for handler: %s. Maybe racing bug.\n", callerName == nullptr ? "<Unknown>" : callerName);
			return nullptr;
		}
...
namespace Resolve
{
    ...
    uintptr_t JMP(uintptr_t module, uintptr_t address); //JMP, CALLS
    uintptr_t MOV32(uintptr_t module, uintptr_t address); //MOV, LEA
    uintptr_t MOV(uintptr_t module, uintptr_t address); //MOV, LEA
    uintptr_t XMM128(uintptr_t module, uintptr_t address); //XMM0-XMM7
}